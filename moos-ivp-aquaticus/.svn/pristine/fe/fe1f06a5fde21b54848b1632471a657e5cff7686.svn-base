...
./launch_vehicle.sh -vs -r1 $RVARGS --start=$SCO_POS --role=ATTACK_E 
echo "Launching Thomas Red-Two"
./launch_vehicle.sh -vt -r2 $RVARGS --start=$THO_POS --role=DEFEND_E 
echo "Launching Ursula Blue-One"
./launch_vehicle.sh -vu -b1 $BVARGS --start=$URS_POS --role=ATTACK_MED 
echo "Launching Valhall Blue-Two"
./launch_vehicle.sh -vv -b2 $BVARGS --start=$VAL_POS --role=DEFEND_MED 
