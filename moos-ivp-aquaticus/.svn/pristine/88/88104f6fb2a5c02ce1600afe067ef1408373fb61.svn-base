...
for ARGI; do
    ...
    elif [ "${ARGI:0:7}" = "--auto=" ] ; then
        AUTO="${ARGI#*--auto=}"
	echo "AUTONOMY set to: " $AUTO
...
if [ "${HELP}" = "yes" ]; then
  echo "$0 [SWITCHES]"
  echo "  XX                  : Time warp"
  echo "  --auto=low/high     : autonomy level"
  echo "  --pid=X              : PID"
  echo "  --cid=X              : CID"
...
#Check to make sure AUTO has been set....otherwise.....print and exit out
if [ $AUTO == "low" ] || [ $AUTO == "high" ] ; then
    echo "AUTONOMY is set properly"
else
    echo "$0 [SWITCHES]"
    echo "AUTONOMY set to " $AUTO " make sure it is high or low"
   exit 0;
fi
...
if [[ -z $NO_HERON ]]; then
  ...
  sleep 1
  # Hal Blue
  ./launch_heron.sh h b2 b1 --auto=$AUTO --cid=$CID $TIME_WARP -s > /dev/null &
  sleep 1
  # Ida Red
