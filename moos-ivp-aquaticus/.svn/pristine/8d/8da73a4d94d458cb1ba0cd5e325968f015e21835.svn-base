...
bool FlagManager::OnStartUp()
{
  ...
  for(p=sParams.begin(); p!=sParams.end(); p++) {
    ...
    else if(param == "post_score")
      handled = handleConfigPostScore(value);
...
//---------------------------------------------------------
// Procedure: handleConfigPostScore()
//      Note: Handles format: x=4,y=8   or 4,8

bool FlagManager::handleConfigPostScore(string str)
{
  string xstr, ystr;
  if(strContains(str, "x=")) {
    xstr = tokStringParse(str, "x");
    ystr = tokStringParse(str, "y");
  }
  else {
    xstr = biteString(str, ',');
    ystr = str;
  ...
  m_post_score_x = atof(xstr.c_str());
  m_post_score_y = atof(ystr.c_str());
  m_post_score = true;
...
bool FlagManager::handleMailFlagGrab(string str, string community)
{
  ...
  for(unsigned int i=0; i<m_flags.size(); i++) {
	...
	m_postscore_last = 0; // trigger immediate post
...
//------------------------------------------------------------
// Procedure: notifyScore()

bool FlagManager::notifyScore()
{
  double elapsed = m_curr_time - m_postscore_last;
  if(elapsed < 10)
    return(true);

  unsigned int max_scores = 0;
  vector<string> leaders;

  string score_summary;
  for (unsigned int i=0; i < m_flags.size(); i++) {
    string team = m_flags[i].get_label();
    string uteam = toupper(team);
    unsigned int grabs = m_map_team_grabs[team];
    unsigned int scores = m_map_team_score[team];
    Notify(uteam + "_GRABS", grabs);
    Notify(uteam + "_SCORES", scores);

    if(scores > max_scores) {
      leaders.clear();
      leaders.push_back(uteam);
      max_scores = scores;
    }
    else if(scores == max_scores)
      leaders.push_back(uteam);
	   
    if(score_summary != "")
      score_summary += " ";
    score_summary += uteam + ":" + uintToString(m_map_team_score[team]);
  }
  ...
  Notify("AQLEADER", tolower(leader));
  Notify("AQLEADERUC", toupper(leader));
  
  if(m_post_score) {
    XYTextBox tbox;
    tbox.setX(m_post_score_x);
    tbox.setY(m_post_score_y);
    tbox.setMsg(score_summary);
    tbox.setFSize(20);
    tbox.setMColor("yellow");
    Notify("VIEW_TEXTBOX", tbox.get_spec());
  }
  
  m_postscore_last = m_curr_time;
  return(true);
}
