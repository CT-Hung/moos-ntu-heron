...
function help(){
    ...
    echo "  --auto=           : Autonomy mode high/low."
    ...
    echo "  --auto=           : Autonomy level high/low"
...
case "$2" in
    r1|red_one)
        ...
        STATION_KEEP_CLOSE_FLAG="120,56"
        RNAME="red_one"
        VPORT="9011"
        SHARE_LISTEN="9311"
    ...
    r2|red_two)
        VTEAM="red"
        START_POS="140,35,240"
        STATION_KEEP_CLOSE_FLAG="120,24"
        RNAME="red_two"
        VPORT="9012"
        SHARE_LISTEN="9312"
...
for arg in "${@:4}"; do
    if [ "${arg}" = "--help" -o "${arg}" = "-H" ]; then
        ...
        CID="${arg#--cid=*}"
        CID=$(printf "%03d" $CID)
    elif [ "${arg:0:7}" = "--auto=" ] ; then
        AUTO="${arg#*--auto=}"
	echo "AUTO set to: " $AUTO
    else
...
if [ "${VTEAM}" = "red" ]; then
    ...
    UNTAG_POS="140,40"
    RETURN_POS="5,0"
    STATION_KEEP_MIDFIELD_ALONE="87,40"
    STATION_KEEP_MIDFIELD_LEFT="95,18"
    STATION_KEEP_MIDFIELD_RIGHT="95,61"
