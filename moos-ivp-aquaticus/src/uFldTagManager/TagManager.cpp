...
bool TagManager::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++) {
    ...
    if((key == "NODE_REPORT") || (key == "NODE_REPORT_LOCAL"))
      ...
      postZonePolys();
    else if(key == "OZONE_TAGS")
      handled = setBooleanOnString(m_ozone_tags, sval);
    else
      handled = false;
...
void TagManager::postTagSummary()
{
  ...
  if(tagged_vnames != m_prev_tagged_vnames) {
    Notify("TAGGED_VEHICLES", tagged_vnames);
    m_prev_tagged_vnames = tagged_vnames;
  }

  string cantag_summary;
  map<string,double>::iterator q;
  for(q=m_map_node_vtags_next_tag.begin();
      q!=m_map_node_vtags_next_tag.end(); q++) {
    string vname = q->first;
    double next_tag = q->second;
    if(next_tag > m_curr_time) {
      if(cantag_summary != "")
	cantag_summary += ",";
      cantag_summary += vname + "=" + doubleToString(next_tag,1);
    }
  }
      
  if(cantag_summary != m_prev_cantag_summary) {
    Notify("CANTAG_SUMMARY", cantag_summary);
    m_prev_cantag_summary = cantag_summary;
  }
}
...
bool TagManager::handleMailVTagPost(string launch_str)
{
  ...
  // Note: Upon a tag request of any type or result, trigger
  // an immediate reposting of tag stats (mikerb Oct 8th, 2023)
  m_time_last_tagstats_post = 0;
...
//   Returns: true if a state changed for any vehicle
//            false if no vehicles changed state

bool TagManager::updateCanTagState(string vname)
{
...
void TagManager::processVTag(VTag vtag)
{
  ...
  if(node_closest_dist <= m_tag_range) {
    ...
    m_map_node_vtags_next_tag[vname] = m_curr_time + m_tag_min_interval;
...
void TagManager::checkForOutOfZoneVehicles()
{
  ...
  for(p=m_map_node_records.begin(); p!=m_map_node_records.end(); p++) {
	...
	m_time_last_tagstats_post = 0; // trigger immediate post
...
//------------------------------------------------------------
// Procedure: postTagStats()

bool TagManager::postTagStats()
{
  double elapsed = m_curr_time - m_time_last_tagstats_post;
  if(elapsed < 10)
    return(true);
  
  map<string, set<string> >::iterator p;
  for(p=m_map_teams.begin(); p!=m_map_teams.end(); p++) {

    set<string> team = p->second;
    set<string>::iterator q;
    for(q = team.begin(); q!=team.end(); q++) {
      string vname = *q;
      m_map_node_vtags_requested[vname];

      // // TODO: Count times pushed off field
      // bool now_onfield = false;
      // if (m_map_node_onfield.count(vname))
      //   now_onfield = m_map_node_onfield[vname];
      Notify(toupper(vname) + "_TIMES_TAGGED",
             m_map_node_vtags_beentagged[vname]);

      Notify(toupper(vname) + "_TAG_REQUESTS",
             m_map_node_vtags_requested[vname]);
      //Notify(toupper(vname) + "_TAG_REQUESTS",
      //       m_map_node_vtags_requested[vname]);

      Notify(toupper(vname) + "_TAGS_ACCEPTED",
             m_map_node_vtags_accepted[vname]);
      Notify(toupper(vname) + "_TAGS_SUCCEEDED",
             m_map_node_vtags_succeeded[vname]);
    }
  }

  m_time_last_tagstats_post = m_curr_time;
...
bool TagManager::buildReport()
{
  ...
  m_msgs << "Tag OZone Enabled:  " << boolToString(m_ozone_tags) << endl << endl;
